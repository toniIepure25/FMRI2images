"""Shortlist-local teacher distillation from rerank space into compact space."""
...
class ShortlistTeacherDistillLoss(nn.Module):
    """Distil rerank-head local ranking preferences into the compact head.

    For each query i in a batch, construct a small candidate subset from:
    - the ground-truth positive i
    - top-K compact candidates
    - top-K rerank candidates

    Then match the compact-head logits to the rerank-head teacher distribution
    on that local subset only.
    """
    ...
    def forward(
        ...
        if rerank_pred.shape[0] != rerank_target.shape[0]:
            raise ValueError("rerank_pred and rerank_target must share batch size")
        if compact_pred.shape[0] != rerank_pred.shape[0]:
            raise ValueError("compact and rerank tensors must share batch size")
        ...
        teacher_scores_det = rerank_pred_f @ rerank_target_f.T
