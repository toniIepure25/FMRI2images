...
\begin{figure}[htbp]
    ...
    \caption{Overview of the complete thesis pipeline. NSD fMRI is preprocessed and mapped by a shared multi-head decoder into three frozen retrieval experts (V61a, V62a, V66a). Their CSLS scores are z-score normalised and combined by a frozen weighted sum, producing the headline SHARED1000 endpoint at $86.3\%$ CSLS Recall@1 (heavy border). The earlier V35 plus N1v28a fixed fusion at $77.2\%$ is retained as a methodological milestone. The reconstruction add-on uses SDXL~1.0 with IP-Adapter visual conditioning and fMRI-predicted CLIP injection on $n{=}141$ SHARED1000 examples.}
...
\begin{itemize}
    ...
    \item a quantitative reconstruction add-on built on Stable Diffusion XL~1.0 with IP-Adapter visual conditioning and fMRI-predicted CLIP injection, evaluated on $n{=}141$ SHARED1000 examples and shown to preserve or improve all evaluated image-fidelity metrics on aggregate (2-way AlexNet(5) identification accuracy $86.2\%$), demonstrating that the predicted brain signal can be used for principled generation without metric regressions;
