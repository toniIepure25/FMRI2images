...
\end{itemize}
This separation is one of the central design insights of the project: a single latent space is not forced to satisfy all design requirements simultaneously. The same multi-head decomposition is reused as the trainable template across the V60--V66 experiment family: each retrieval expert is a fresh instance of this template, trained independently end-to-end with its own target and its own optimisation schedule. The final exported system therefore preserves the compact / rerank / rich decomposition at the per-expert level and adds a frozen score-level combination across three such experts at inference time; the cross-architecture triple score fusion that yields the primary SHARED1000 retrieval result is described in Section~\ref{chap:experiments}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{model_architecture.png}
    \caption{Trainable multi-head decoder shared by the V60--V66 experiment family. A residual MLP encoder operates on the preprocessed voxel vector and feeds three task-specific heads (compact for shortlist, rerank for shortlist-local correction, rich for regression-style targets and qualitative decoding). This is the trainable architecture; the inference-time score fusion across V61a, V62a, and V66a that yields the primary SHARED1000 retrieval result is described in Section~\ref{chap:experiments} and depicted separately in Figure~\ref{fig:triple_fusion_architecture}.}
    \label{fig:model_architecture}
\end{figure}

\section{Inference and Score Fusion}

At inference time, the compact head first retrieves a shortlist of candidate images. Let $\mathcal{S}_K(x_i)$ denote the top-$K$ shortlist obtained from compact retrieval. In the earlier compact--rerank design stage, the rerank head rescales those candidates locally. If $s_c(i,j)$ is the compact score and $s_r(i,j)$ is the rerank score for candidate $j$ in the shortlist of sample $i$, the shortlist-local fused score can be written as
\begin{equation}
...
A first frozen fixed-fusion system extends this idea with an earlier complementary baseline expert. Using z-score-normalized expert scores, the corresponding score combination can be written as
\begin{equation}
...
\end{equation}
with $\widetilde{s}_{c,\mathrm{CSLS}}$ denoting the normalized compact-CSLS score from V35 and $\widetilde{s}_{\ell,\mathrm{CSLS}}$ denoting the normalized CSLS score from the earlier complementary baseline expert N1v28a. The rerank branch remains methodologically important and still informs the architecture; the V35 plus baseline-expert combination is retained as a milestone result that validates the compact--rerank--rich decomposition under expert fusion.
