...
\label{sec:defense_summary}

Table~\ref{tab:defense_summary} consolidates the four lines of evidence in this thesis with their supporting artifacts and scope limits. Table~\ref{tab:artifact_map} pairs each claim with the concrete repository path that produced it; both tables are intended as a single-glance reference during the oral defence.
...
\toprule
\textbf{Claim} & \textbf{What the thesis demonstrates} & \textbf{Evidence} & \textbf{Scope / limitation of the claim} \\
\midrule
Retrieval (headline) & A frozen score-level fusion of three independently trained retrieval experts reaches $86.3\%$ CSLS R@1 on SHARED1000. & Tables~\ref{tab:final_results} and~\ref{tab:expert_complementarity}; numbers from Table~\ref{tab:artifact_map}. & Single subject (subj01), single gallery (SHARED1000); per-trial fused-rank export not preserved $\Rightarrow$ no bootstrap CI in this thesis. \\
CSLS + complementarity & CSLS reduces hubness substantially in the analysed expert and the fusion gain is larger than what redundant ensembling would predict. & Table~\ref{tab:manifold_evidence}, near-miss-regime analysis in \S\ref{sec:why_triple_fusion_anchor}. & Manifold evidence is V62a-only; complementarity claim is aggregate, not per-trial. \\
Reconstruction add-on & SDXL + IP-Adapter (anchor from triple-fusion top-1) + fMRI-predicted CLIP injection yields modest improvements in SSIM, PSNR, CLIP-I, and AlexNet(5) identification on 141 SHARED1000 examples; other metrics are essentially unchanged. & Table~\ref{tab:diffusion_summary}; numbers from Table~\ref{tab:artifact_map}. & Not a pure fMRI-only reconstruction: the IP-Adapter anchor carries most of the low-level structure. 5-way anchor-vs-CLIP ablation not yet run. \\
Ethical scope & The system decodes \emph{viewed external stimuli} from consenting NSD subjects against a fixed gallery; it is not a general "mind reading" system. & \S\ref{sec:ethics} (Ethical Considerations and Scope of Brain Decoding). & The result does not extrapolate to ecological settings, decoding of mental imagery, or non-consensual neural inference. \\
\bottomrule
\end{tabularx}
\caption{Defense summary. Each row pairs a thesis-level claim with the supporting evidence and the explicit scope under which the claim is intended to be defended.}
...
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.4cm} >{\raggedright\arraybackslash\ttfamily}X@{}}
\toprule
\normalfont\textbf{Artifact} & \normalfont\textbf{Repository path (relative to project root)} \\
\midrule
V61a expert (token, MC-TTA) & configs/ experiments/ V61a\_finetune\_difflr.yaml \\
V62a expert (768-D CLS) & configs/ experiments/ V62a\_cls\_retrieval\_768d.yaml \\
V66a expert (768-D ROI) & configs/ experiments/ V66a\_roi\_pretrain.yaml \\
Triple-fusion script & scripts/ evaluation/ triple\_fusion\_shared1000\_fixed.py \\
Triple-fusion metrics & experimental\_results/ fusion\_triple\_86/ triple\_fusion\_shared1000\_86.json \\
Reconstruction pipeline & scripts/ evaluation/ run\_sdxl\_ipadapter\_reconstruction.py \\
Reconstruction metrics & experimental\_results/ reconstruction\_sdxl\_ipa/ metrics\_summary.json \\
Reconstruction per-example CSV & experimental\_results/ reconstruction\_sdxl\_ipa/ per\_example.csv \\
Qualitative retrieval panels & thesis/ v62\_thesis/ query\_\{8262,21279,55649,8509\}.png \newline \normalfont(per-trial GT ranks in)\ttfamily\ thesis/ v62\_thesis/ panel\_metadata.json \\
V62a manifold report & reports/ manifold\_analysis/ 20260514\_V62a\_shared1000\_subj01\_hardened\_with\_ids/ \\
\bottomrule
\end{tabularx}
\caption{Artifact and reproducibility map. Path components are spaced with thin breaks so they wrap cleanly inside the table column; the actual filesystem paths concatenate the segments shown. Every number in Chapters~5 and~6 of this thesis can be traced to one of the paths listed here.}
