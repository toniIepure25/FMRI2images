...
subjects = cfg.get("data", {}).get("subjects") or []
print(" ".join(str(s) for s in subjects))
PY
}
...
declare -A CONFIGS=(
    ...
    [N1v26c]="N1v26c_token_targets_improved.yaml|v26c: V26a + R-Drop + label smoothing + slerp MixCo + stronger regularization"
    [N1v26d]="N1v26d_kappa_unlocked.yaml|v26d: V26c + kappa_reg=false (unlock kappa in 197K-D token space)"
    [N1v27a]="N1v27a_bigg_tokens.yaml|v27a: ViT-bigG/14 257×1280 token targets (V26a recipe, queue 2048)"
    [N1v28a]="N1v28a_dual_head.yaml|v28a: Dual-head MindEye-style (L2-norm contrastive + un-normalised MSE, ViT-L/14)"
    [N1v28b]="N1v28b_dual_head_bigg.yaml|v28b: Dual-head + ViT-bigG/14 tokens (full MindEye recipe)"
    [N1v29a]="N1v29a_cross_subject.yaml|v29a: Cross-subject pre-training (4 subj, 768-D CLS, ~862M params)"
    [N1v29b]="N1v29b_finetune.yaml|v29b: Fine-tune V29a encoder + dual-head + 197K-D token targets"
    ...
    [V30d_short]="V30d_short.yaml|v30d short: 20-epoch rerank-head learning run without shared1000 eval"
    [V30d_full]="V30d_full.yaml|v30d full: full training schedule with post-training shared1000 evaluation"
    [V30e]="V30e_rerank_head_2048.yaml|v30e: rerank head ablation with rerank_dim=2048"
    [V30f]="V30f_fused_checkpoint.yaml|v30f: V30e with fixed score fusion evaluation and fused-R@1 checkpointing"
    [V31]="V31_compact1024_fused.yaml|v31: widen compact retrieval head to 1024 while keeping rerank=2048 and fused checkpointing"
    [V32]="V32_pca_rerank_2048.yaml|v32: keep V30e architecture but replace random-projection rerank targets with train-only PCA"
    ...
    [V38]="V38_legacy_compact_distill.yaml|v38: V35 base + top-k frozen legacy N1v28a distillation into the compact head"
    [V39]="V39_union_shortlist_residual_reranker.yaml|v39: candidate-level residual reranker over union shortlist (post-hoc, uses train_union_shortlist_reranker.py)"
    [V40]="V40_oof_tri_gate.yaml|v40: OOF-trained union-shortlist reranker (post-hoc, train/eval mismatch fix)"
    [V41]="V41_oof_union_vmf_resolver.yaml|v41: true-OOF union-shortlist vMF evidence resolver with pairwise shortlist loss (post-hoc)"
    [V42]="V42_multi_hypothesis_vmf_retrieval.yaml|v42: 4-component mixture-vMF compact retrieval head with tri-fused checkpointing"
    ...
    [V45a]="V45a_all8_fusion_distill_smoke.yaml|v45a: all-8 retrieval student distilled from the frozen best-system fusion ranking topology over compact and legacy target spaces"
    [V45b]="V45b_all8_fusion_distill_full.yaml|v45b: full all-8 retrieval student with stronger frozen-fusion ranking distillation and shared1000 evaluation"
...
ensure_preextracted_subjects "$SUBJECTS"
...
for exp_id in "${EXPERIMENT_ORDER[@]}"; do
    ...
    config_subjects="$(get_config_subjects "$config_path")"
    if [[ -n "$config_subjects" ]]; then
        echo "Config subjects: ${config_subjects}"
        ensure_preextracted_subjects "$config_subjects"
    fi
