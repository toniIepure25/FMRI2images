...
const LABEL_STYLES: Record<RetrievedImage['label'], { text: string; className: string }> = {
  correct: { text: 'Correct', className: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' },
  semantic_neighbor: {
    text: 'Semantic neighbor',
    className: 'bg-violet-500/20 text-violet-200 border-violet-500/35',
  },
  distractor: { text: 'Distractor', className: 'bg-slate-500/20 text-slate-300 border-slate-500/35' },
};
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="glass-panel p-5">
        ...
        <p className="mt-2 text-xs leading-relaxed text-slate-400">
          Even when top-1 is wrong, top-<span className="font-mono text-slate-300">k</span> often reveals semantic
          proximity in CLIP space—nearby items share category or visual structure despite pixel differences.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-stretch">
        <div className="glass-panel flex w-full flex-shrink-0 flex-col p-4 lg:w-[320px]">
          ...
          <div className="relative flex-1 overflow-hidden rounded-xl border-2 border-brain-accent/35 bg-black/50 shadow-[0_0_32px_rgba(0,212,255,0.08)]">
            <SafeImg
              ...
              alt="Target"
              className="h-full min-h-[240px] w-full object-cover lg:min-h-[280px]"
            />
          </div>
        ...
        <div className="min-w-0 flex-1 space-y-4">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Top-5 candidates</p>
          <div className="flex flex-wrap gap-4 lg:flex-nowrap">
            {top5.map((item) => {
              ...
              return (
                <div
                  key={item.rank}
                  className={`glass-panel-hover flex min-w-[140px] flex-1 flex-col overflow-hidden p-3 ${
                ...
                >
                  <div className="relative aspect-square overflow-hidden rounded-lg border border-brain-border/40 bg-black/40">
                    ...
                    <div className="absolute left-2 top-2 rounded-md border border-white/20 bg-black/70 px-2 py-0.5 font-mono text-[10px] font-bold text-white backdrop-blur-sm">
                      #{item.rank}
                    </div>
                  </div>
                  <div className="mt-3 space-y-2">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className="rounded-md border border-white/10 bg-white/5 px-2 py-0.5 font-mono text-[10px] text-cyan-200">
                    ...
                    <span className={`inline-flex rounded-md border px-2 py-0.5 text-[10px] font-medium ${ls.className}`}>
                      {ls.text}
                    </span>
                  </div>
                </div>
