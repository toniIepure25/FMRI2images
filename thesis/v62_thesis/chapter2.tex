...
For the reconstruction add-on the thesis additionally reports image-fidelity scores that have become standard in the fMRI-to-image literature \cite{Scotti2023,Scotti2024,Ozcelik2023}: pixel correlation, SSIM, PSNR, LPIPS, CLIP image similarity, and feature-space cosine similarity at two depths of an ImageNet-pretrained AlexNet. Following the convention used by MindEye and Brain Diffuser, AlexNet(2) denotes cosine similarity between the activations of the second convolutional block (early, mostly texture/edge features), and AlexNet(5) denotes cosine similarity between the activations of the fifth convolutional block (mid-level, more semantically organized features). All AlexNet activations are flattened and $\ell_2$-normalized before the cosine product so that the metric is scale-invariant. These metrics are auxiliary, not retrieval endpoints. For the final reconstruction add-on the auxiliary image-fidelity metrics are reported on $141$ SHARED1000 examples; the 2-way identification analysis based on AlexNet features is additionally reported over the $137$ non-perfect cases, excluding the perfect-bucket examples on which the reconstruction and reference comparison would be degenerate. Earlier 16-example qualitative panels are treated only as preliminary diagnostic visualisations and are not the final quantitative reconstruction benchmark.
