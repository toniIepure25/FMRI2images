...
The $+3.2$ percentage-point gain over the strongest single expert is reported as an aggregate point estimate over $1000$ SHARED1000 trials, alongside a $+1.1$ percentage-point gain at $R@5$ and a parallel rise in $\mathrm{MRR}$ from $0.892$ to $0.914$ (Table~\ref{tab:expert_complementarity}). The complementarity claim is therefore supported at the aggregate-metric level only. A publication-oriented extension should re-export per-trial fused ranks across SHARED1000 and add bootstrap confidence intervals together with a paired correct-vs-wrong transition table between the strongest single expert and the fusion. The frozen recipe $0.7/0.1/0.2$ with $k=3$ was itself selected by a small validation-side simplex sweep before SHARED1000 evaluation.
