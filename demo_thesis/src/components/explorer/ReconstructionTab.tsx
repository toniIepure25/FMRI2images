...
function SafeImg({ src, alt, className }: { src: string; alt: string; className?: string }) {
  ...
  if (!src || !ok) {
    return (
      <div className={`flex items-center justify-center bg-[#050914] text-xs text-slate-600 ${className ?? ''}`}>\u2014</div>
    );
  }
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="glass-panel p-5">
        ...
        <p className="mt-2 text-xs leading-relaxed text-slate-400">
          ...
          inter-ROI disagreement (\u03B4) into guidance scale, sampling steps, and optional ensembling\u2014trading fidelity
          against diversity when brain evidence is ambiguous.
        </p>
      </div>
          ...
          <button
            ...
            className={`rounded-lg border px-4 py-2 text-xs font-medium transition ${
              preset === p
                ...
                : 'border-brain-border/50 bg-brain-panel/60 text-slate-400 hover:border-brain-accent/25 hover:text-slate-200'
            }`}
          >
            {PRESET_META[p].label}
          </button>
      ...
      <p className="text-[11px] leading-relaxed text-slate-500">{PRESET_META[preset].caption}</p>
      ...
      {hasDiffusionPair && (
        <div className="glass-panel p-5">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Diffusion progression</p>
      ...
      <div className="glass-panel p-5">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">DUA-CFG parameters</p>
        <dl className="mt-3 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
          <div className="rounded-lg border border-white/5 bg-slate-950/50 px-3 py-2">
            <dt className="text-[10px] text-slate-500">Guidance scale</dt>
            <dd className="mt-1 font-mono text-white">{case_.duaCfg.guidanceScale.toFixed(1)}</dd>
          </div>
          <div className="rounded-lg border border-white/5 bg-slate-950/50 px-3 py-2">
            <dt className="text-[10px] text-slate-500">Diffusion steps</dt>
            <dd className="mt-1 font-mono text-white">{case_.duaCfg.diffusionSteps}</dd>
          </div>
          <div className="rounded-lg border border-white/5 bg-slate-950/50 px-3 py-2">
            <dt className="text-[10px] text-slate-500">Ensemble K</dt>
            <dd className="mt-1 font-mono text-white">{case_.duaCfg.ensembleK}</dd>
          </div>
          <div className="rounded-lg border border-white/5 bg-slate-950/50 px-3 py-2">
            <dt className="text-[10px] text-slate-500">Abstain</dt>
    ...
    <div className={`glass-panel overflow-hidden ${accent} border`}>
      ...
      <div className="relative aspect-square bg-[#050914]">
        {empty ? (
          ...
          <SafeImg src={src} alt={title} className="h-full w-full object-cover" />
        )}
      </div>
    </div>
  );
