...
Third, \emph{SDXL with IP-Adapter conditioning preserves or improves all metrics on aggregate}. The SDXL pipeline replaces SD~2.1 with Stable Diffusion XL~1.0, uses IP-Adapter cross-attention on the anchor image for principled visual conditioning, and injects the fMRI-predicted CLIP embedding into SDXL's native ViT-L/14 text-encoder pathway (which shares the same 768-D embedding space as the decoder output). Strength, guidance scale, and number of steps are derived per example from a confidence score $c \in [0, 1]$ defined as the margin between the CSLS top-1 and CSLS top-2 fused scores for the query, linearly mapped to the strength interval $[0, 0.30]$. Table~\ref{tab:diffusion_summary} reports the full per-metric outcome across all $n{=}141$ examples: SSIM gains $+0.012$, PSNR gains $+0.063$\,dB, CLIP image similarity gains $+0.005$, and LPIPS improves by $-0.0003$ (lower is better), while pixel correlation and AlexNet features remain essentially flat. The pattern represents a uniform improvement over the retrieval anchor: the SDXL and IP-Adapter combination generates images that are structurally and semantically closer to the ground truth without trading off any metric dimension. Additionally, the 2-way identification accuracy using AlexNet(5) features reaches $86.2\%$ across the 137 non-perfect examples, providing a directly MindEye2-comparable reconstruction quality metric.
...
The bucketed analysis refines the picture. Perfect cases ($n{=}4$) remain perfect, validating the anchor-first controller. For the good bucket ($n{=}117$), diffusion improves SSIM ($+0.013$), PSNR ($+0.065$\,dB), and CLIP image similarity ($+0.005$), while pixel correlation and AlexNet features are essentially flat. For the near-good bucket ($n{=}13$), the strongest CLIP image-similarity gain appears ($+0.014$), with SSIM ($+0.009$) and PSNR ($+0.054$\,dB) also improving. For the hard bucket ($n{=}7$), SSIM gains $+0.011$, PSNR gains $+0.055$\,dB, and CLIP image similarity gains $+0.006$. Across all three non-perfect buckets, every metric either improves or stays flat; no bucket shows the CLIP image-similarity regression observed with the earlier SD~2.1 pipeline. This uniform improvement is attributable to two changes: SDXL's stronger image prior generates higher-fidelity textures than SD~2.1, and IP-Adapter cross-attention provides a principled visual conditioning pathway that does not conflict with the semantic CLIP injection. The evaluation on $n{=}137$ non-perfect examples makes these conclusions statistically defensible, unlike the earlier $n{=}12$ non-perfect subset where outlier-driven mean--median divergence obscured the signal.
