...
\begin{itemize}
    ...
    \item extending the SDXL + IP-Adapter reconstruction pipeline with two-stage diffusion (sketch then refine), test-time CLIP-guided sampling, and VDVAE-based low-level structure regression for stronger pixel-level fidelity;
...
In conclusion, the thesis provides a reproducible, technically mature, and scientifically grounded step toward reliable fMRI-to-image decoding. Its main value lies not only in the final metric values, but also in the clarity of the resulting design principles: compact shortlist retrieval, dedicated reranking, and rich regression supervision should be treated as distinct roles, and complementary retrieval experts can be profitably fused after training when they demonstrably capture different parts of the signal. The SDXL + IP-Adapter diffusion add-on strengthens this conclusion rather than replacing it: across $n{=}137$ non-perfect SHARED1000 examples, the pipeline preserves or improves all evaluated metrics on aggregate, with 2-way AlexNet(5) identification accuracy of $86.2\%$, demonstrating that the fMRI-predicted CLIP embedding can be used for principled image generation without metric regressions. Exact retrieval remains the most trustworthy endpoint, but the reconstruction stage now operates as a modest improvement rather than as a trade-off.
