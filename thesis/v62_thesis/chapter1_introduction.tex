...
\begin{figure}[htbp]
    ...
    \caption{Overview of the thesis pipeline. After shared preprocessing, three independently trained retrieval experts produce CSLS-corrected score matrices, which are $z$-score normalised and combined by a frozen weighted sum to obtain the primary SHARED1000 retrieval endpoint. The fusion is applied only at inference time and only at score level; it does not define a jointly trained model or a unified fused latent embedding. The reconstruction branch is a secondary anchor-dependent add-on conditioned on the retrieved top-1 image and an fMRI-predicted CLIP signal.}
...
\begin{itemize}
    \item the design and implementation of a complete, reproducible fMRI-to-latent decoding pipeline built around NSD, CLIP-aligned targets, and retrieval-based evaluation;
    \item a systematic validation and debugging program addressing sample--target alignment, split integrity, checkpoint selection, and retrieval geometry;
    \item the empirical justification for separating compact retrieval, shortlist-local reranking, and rich latent regression into distinct roles rather than forcing a single latent space to serve all objectives equally well;
    \item the validation of frozen inference-time expert fusion as a practical way to improve retrieval once complementary experts have been trained independently;
    \item the final exported cross-architecture triple-fusion retrieval system, which constitutes the main quantitative contribution of the thesis;
    \item a secondary SDXL + IP-Adapter reconstruction add-on evaluated conservatively as an anchor-dependent downstream extension rather than as a pure fMRI-only reconstruction system;
    \item the explicit documentation of negative results and unsuccessful directions, which narrows the future search space and improves the scientific value of the work.
\end{itemize}
