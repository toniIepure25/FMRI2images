...
| v26d | N1v26d | V26c + kappa_reg disabled (unlock kappa in 197K-D token space) | **In progress** |
...
- v26d: single-variable ablation — V26c with kappa_reg.enabled=false; hypothesis: removing the regularizer lets kappa rise from the softplus floor (~1.54) to reflect actual model confidence, potentially breaking the 69.6% CSLS ceiling; softplus floor (1.0) + AMP clamp (500) provide sufficient bounds; in progress
